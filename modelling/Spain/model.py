...
def read_and_format_data(datadir, outdir):
        ...
        #Convert to datetime
        epidemic_data['date']=pd.to_datetime(epidemic_data['dateRep'])

        #Mobility data
        mobility_data = pd.read_csv(datadir+'Global_Mobility_Spain.csv')
        #Convert to datetime
       ...
       'residential_percent_change_from_baseline']
        ...
        for m in range(len(mob_sectors)):
            ...
            plt.plot(np.arange(y.shape[1]),y[m,:], label = mob_labels[sector], linewidth=1)
        #Reverse residential
        y[5,:] = -y[5,:]
